...
export function generateCssVars(token: any, prefix = 'ant', keys?: string[]) {
  const pattern =
  ...
  const cssVars = keys.map(
    key =>
      `--${prefix}-${kebabCase(key)}: ${token[key]}${
        pxPattern.test(key) ? 'px' : ''
      }`
  )
  return cssVars.join(';\n')
}
...
export function generateColorCssVars(token: any, prefix = 'ant') {
  ...
  const keys = Object.keys(token).filter(
    key =>
      !pattern.test(key) && key.startsWith('color') && !excludedKeys.test(key)
  )
  const cssVars = keys.map(
    key =>
      `--${prefix}-${kebabCase(key)}: ${token[key]}${
...
export function getColorCssVars(token: any, prefix = 'ant') {
  const keys = Object.keys(token).filter(
    k => k.startsWith('color') && !excludedKeys.test(k)
  )
